...
void init(int windowWidth, int windowHeight) {
    ...
    SDL_SetWindowTitle(window, "Bubble Bobble Breaker");
...
void freedragon(SDL_Texture *texturedragon) {
    /** @brief libère la mémoire allouée à la texture (utilisée pour dans la fonction sprite() pour afficher une image)
     *  @param texture la texture utilisée pour afficher l'image
     */
    if (texturedragon) SDL_DestroyTexture(texturedragon);
}
...
void changeColor(int r, int g, int b, int alpha) {//int alpha remettre SDL_ALPHA_OPAQUE 255 ?
     ...
     *  @param alpha gère la transparence entre 0 et 225
     */
    if (SDL_SetRenderDrawColor(renderer, r, g, b, alpha)) {
        SDL_Log("ERREUR : changement de couleur > %s\nParametres passes %d , %d, %d, %d\n",SDL_GetError(), r, g, b, alpha);
        freeAndTerminate();
    }
}
